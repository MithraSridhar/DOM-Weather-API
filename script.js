...
async function restCountriesAPI(){
    ...
    console.log(city1)
        ...
        let cityFlag="https://upload.wikimedia.org/wikipedia/commons/thumb/3/3f/Flag_of_Afghanistan_%282004%E2%80%932021%2C_variant%29.svg/800px-Flag_of_Afghanistan_%282004%E2%80%932021%2C_variant%29.svg.png?20191102020257";
    ...
    document.getElementsByClassName("card-img-top")[0].setAttribute('src',cityFlag)
    document.getElementsByClassName("card-title")[0].innerText=`Capital:${cityCapitol}` 
    document.getElementsByClassName("card-text")[0].innerHTML=`Region:${cityRegion} <br/> Country Code:${cityCode}` 
    document.getElementsByClassName("btn btn-primary")[0].addEventListener('click',()=>{
      
    let city1weather =  document.getElementsByClassName("card-header")[0].textContent;
    ...
    console.log(city2Flag)   
     ...
     document.getElementsByClassName("card-img-top")[1].setAttribute('src',city2Flag)
     document.getElementsByClassName("card-title")[1].innerText=`Capital:${city2Capitol}` 
     document.getElementsByClassName("card-text")[1].innerHTML=`Region:${city2Region} <br/> Country Code:${city2Code}` 
     document.getElementsByClassName("btn btn-primary")[1].addEventListener('click',()=>{
       
     let city2weather =  document.getElementsByClassName("card-header")[1].textContent;
     ...
     document.getElementsByClassName("card-img-top")[2].setAttribute('src',city3Flag)
     document.getElementsByClassName("card-title")[2].innerText=`Capital:${city3Capitol}` 
     document.getElementsByClassName("card-text")[2].innerHTML=`Region:${city3Region} <br/> Country Code:${city3Code}` 
     document.getElementsByClassName("btn btn-primary")[2].addEventListener('click',()=>{
       
     let city3weather =  document.getElementsByClassName("card-header")[2].textContent;
...
async function loadTemperature(cityName){
    ...
    console.log(data1)
     console.log(`${cityName}'s maximum temperature is ${data1.main.temp_max.toFixed()}°C`)
     if(cityName=="Afghanistan")
     {
     document.getElementById("weatherDisplay").innerHTML=`<b>${cityName}'s maximum temperature is ${data1.main.temp.toFixed()}°C</b>`
    }
    else if(cityName=="Uzbekistan")
    {
    document.getElementById("weatherDisplay2").innerHTML=`<b>${cityName}'s maximum temperature is ${data1.main.temp.toFixed()}°C</b>`
   }
   else if(cityName=="Albania")
    {
    document.getElementById("weatherDisplay3").innerHTML=`<b>${cityName}'s maximum temperature is ${data1.main.temp.toFixed()}°C</b>`
   }
